...
class UNET_1D(nn.Module):
    def __init__(self, input_dim, layer_n, kernel_size, depth, num_points=1024):
        ...
        self.outcov = nn.Conv1d(
            self.layer_n, 3, kernel_size=self.kernel_size, stride=1, padding=3
        )
    ...
    def forward(self, x):
        assert x.shape[1] == 2
        _flag = False
        # if x.shape[-3] != self.num_points:
        #     _flag = True
        #     old_len = x.shape[-1]
        #     additional_length = self.num_points - old_len
        #     zero_tensor = torch.zeros(x.shape[0], x.shape[1], additional_length).to(
        #         x.device
        #     )
...
if __name__ == "__main__":
    model = UNET_1D(2, 128, 7, 3)  # (input_dim, hidden_layer, kernel_size, depth)
    data = torch.randn(1, 2, 6000)
